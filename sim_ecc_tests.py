...
# The dcrypto P-256 firmware uses word-addressed DMEM pointers.
# OTBN assembly (if used) needs byte-addressed pointers.
...
def _set_dmem_addressing(byte_addressing):
    global DMEM_BYTE_ADDRESSING
    global dmem_mult
    DMEM_BYTE_ADDRESSING = bool(byte_addressing)
    dmem_mult = 32 if DMEM_BYTE_ADDRESSING else 1
...
def load_program_hex():
    ...
    start_addr_dict = {
        "p256init": P256INIT_START_ADDR,
        "p256isoncurve": P256ISONCURVE_START_ADDR,
        "p256scalarmult": P256SCALARMULT_START_ADDR,
        "p256sign": P256SIGN_START_ADDR,
        "p256verify": P256VERIFY_START_ADDR,
    }
    stop_addr_dict = {
        "p256init": P256INIT_STOP_ADDR,
        "p256isoncurve": P256ISONCURVE_STOP_ADDR,
        "p256scalarmult": P256SCALARMULT_STOP_ADDR,
        "p256sign": P256SIGN_STOP_ADDR,
...
def load_program_asm():
    ...
    start_addr_dict = {
        "p256init": function_addr["p256init"],
        "p256isoncurve": function_addr["p256isoncurve"],
        "p256scalarmult": function_addr["p256scalarmult"],
        "p256sign": function_addr["p256sign"],
        "p256verify": function_addr["p256verify"],
    }
    stop_addr_dict = {
        "p256init": function_addr["MulMod"] - 1,
        "p256isoncurve": function_addr["ProjAdd"] - 1,
        "p256scalarmult": len(ins_objects) - 1,
        "p256sign": function_addr["p256scalarbasemult"] - 1,
...
def run_test_scalarmul_random():
    ...
    randy = int(randkey.public_key().pointQ.y.to_bytes(32).hex(), 16)
    randk = int(
        Integer.random_range(min_inclusive=1, max_exclusive=P256_CURVE_ORDER)
        .to_bytes(32)
        .hex(),
        16,
    )
...
def main():
    ...
    load_program_hex()
    # load_program_asm()
    # load_program_otbn_asm()  # requires asm/p256.s (OTBN P-256 asm) which doesn't exist
