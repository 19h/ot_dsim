...
def _shift_u256(value, shift_right, shift_bytes):
    shift_bits = shift_bytes * 8
    if shift_right:
        return c_backend.shr_u256(value, shift_bits)
    return c_backend.shl_u256(value, shift_bits)
...
def _get_single_reg_and_index_imm(asm_str):
    ...
    if len(substr) != 2:
        raise SyntaxError(
            "Syntax error in parameter set. Expected register and indexed immediate"
        )
...
def _get_double_reg_with_imm(asm_str):
    ...
    if len(substr) != 3:
        raise SyntaxError(
            "Syntax error in parameter set. Expected two reg references and immediate"
        )
...
def _get_single_reg_with_section(asm_str):
    ...
    if asm_str.endswith("u"):
        ...
        raise SyntaxError(
            "Expecting 'u' or 'l' at end of register reference with section indication"
        )
...
def _get_reg_with_limb(asm_str):
    ...
    if len(substr) != 2:
        raise SyntaxError(
            "Syntax error in parameter set. Expected reference to 16 bit "
...
def _get_reg_limb_and_imm(asm_str):
    ...
    if len(substr) != 2:
        raise SyntaxError(
            "Syntax error in parameter set. Expected reg with limb + immediate"
        )
...
class Ins(object):
    ...
    def get_bin_with_separators(self):
        return (
            "|"
            + self.get_byte_bin_str(3)
            + "|"
            + self.get_byte_bin_str(2)
            + "|"
            + self.get_byte_bin_str(1)
            + "|"
            + self.get_byte_bin_str(0)
            + "|"
        )
...
class GIStdShift(GIStd):
    ...
    def dec(self):
        super().dec()
        self.shift_bytes = self.get_gen_bit_slice(
            self.imm, self.SHIFT_IMM_POS, self.SHIFT_IMM_LEN
        )
        self.shift_right = bool(
            self.get_gen_bit_slice(self.imm, self.SHIFT_DIR_POS, self.SHIFT_DIR_LEN)
        )
...
class IAdd(GIStdShift):
    ...
    def execute(self, m):
        ...
        if self.MNEM.get(self.fun) == "add":
            ...
            m.setx_c_z_m_l(flag_val)
        elif self.MNEM.get(self.fun) == "addcx":
            res, carry_out = c_backend.add_u256(
                m.get_reg(self.rs1), rs2op, bool(m.get_flag("XC"))
            )
            flag_val = res + (carry_out << m.XLEN)
            m.stat_record_flag_access("x", self.MNEM.get(self.fun))
            m.setx_c_z_m_l(flag_val)
...
class IAddm(GIStdShift):
    ...
    def execute(self, m):
        rs2op = _shift_u256(m.get_reg(self.rs2), self.shift_right, self.shift_bytes)
        sum_low, carry_out = c_backend.add_u256(m.get_reg(self.rs1), rs2op)
        sum_full = sum_low + (carry_out << m.XLEN)
        res = sum_full % m.get_reg("mod")
        m.set_z_m_l(res)
        m.set_reg(self.rd, res)
        trace_str = self.get_asm_str()[1]
        return trace_str, None
...
class ISub(GIStdShift):
    ...
    def execute(self, m):
        ...
        if self.MNEM.get(self.fun) == "sub":
            res, _ = c_backend.sub_u256(m.get_reg(self.rs1), rs2op)
            m.set_flag("C", b)
            m.stat_record_flag_access("n", self.MNEM.get(self.fun))
            m.set_z_m_l(res)
        elif self.MNEM.get(self.fun) == "subb":
            res, _ = c_backend.sub_u256(
                m.get_reg(self.rs1), rs2op, bool(m.get_flag("C"))
            )
            m.set_flag("C", b)
            m.stat_record_flag_access("n", self.MNEM.get(self.fun))
            ...
            res, _ = c_backend.sub_u256(m.get_reg(self.rs1), self.imm)
            m.set_flag("C", b)
            m.stat_record_flag_access("n", self.MNEM.get(self.fun))
            m.set_z_m_l(res)
...
class ISubm(GIStdShift):
    ...
    def execute(self, m):
        rs2op = _shift_u256(m.get_reg(self.rs2), self.shift_right, self.shift_bytes)
        diff_low, borrow_out = c_backend.sub_u256(m.get_reg(self.rs1), rs2op)
        diff_full = diff_low - (borrow_out << m.XLEN)
        res = diff_full % m.get_reg("mod")
...
class IMul128(GIStd):
    ...
    def dec(self):
        super().dec()
        self.r1_upper = bool(
            self.get_gen_bit_slice(self.fun, self.R1UL_POS, self.R1UL_LEN)
        )
        self.r2_upper = bool(
            self.get_gen_bit_slice(self.fun, self.R2UL_POS, self.R2UL_LEN)
        )
    ...
    def execute(self, m):
        op1_shift = (m.XLEN // 2) if self.r1_upper else 0
        op2_shift = (m.XLEN // 2) if self.r2_upper else 0
...
class INot(GIStdShift):
    ...
    def execute(self, m):
        rsop = _shift_u256(m.get_reg(self.rs2), self.shift_right, self.shift_bytes)
        res = c_backend.not_u256(rsop)
...
class ISel(GIStd):
    ...
    def get_asm_str(self):
        asm_str = (
            self.MNEM.get((self.fun, self.imm))
            + " r"
            + str(self.rd)
            + ", r"
            + str(self.rs1)
...
class IRshi(GIStd):
    ...
    def execute(self, m):
        upper = c_backend.shl_u256(m.get_reg(self.rs2), m.XLEN - self.imm)
        lower = c_backend.shr_u256(m.get_reg(self.rs1), self.imm)
...
class ICmp(GIStd):
    ...
    def execute(self, m):
        cmp_res = c_backend.cmp_u256(m.get_reg(self.rs1), m.get_reg(self.rs2))
        if self.MNEM.get(self.fun) == "cmp":
            m.stat_record_flag_access("n", self.MNEM.get(self.fun))
            m.set_flag("Z", cmp_res == 0)
            m.set_flag("C", cmp_res < 0)
...
class ILdi(GIWideImm):
    ...
    def convert_otbn(self, addr):
        ...
        return [
            IOtAddi(3, 0, self.rd, self.ctx),
            IBnLid(3, 0, 0, 0, self.idx * dmem_mult, self.ctx),
        ]
...
class IMovLdr(GIStd):
    ...
    def get_asm_str(self):
        ...
        if self.MNEM.get(self.fun) == "mov":
            asm_str += (
                self.MNEM.get(self.fun) + " r" + str(self.rd) + ", r" + str(self.rs)
            )
        else:
    ...
    def convert_otbn(self, addr):
        ...
        if self.MNEM.get(self.fun) == "ldr":
            ...
            logging.info(
                "OTBN conversion: Mapping rfp limb "
                + str(self.rd_limb)
                + " on GPR x"
                + str(xd)
            )
...
class IMovi(GIWideImm):
    ...
    def dec(self):
        super().dec()
        self.slice = self.get_gen_bit_slice(
            self.funi, self.S_POS_IN_FUNI, self.S_LEN_IN_FUNI
        )
...
class ISt(GIStd):
    ...
    def convert_otbn(self, addr):
        ...
        offset = 0
        logging.info(
            "OTBN conversion: Mapping dmp limb "
            + str(self.limb_src)
            + " on GPR x"
            + str(xd)
        )
        ...
        if self.inc_src and self.inc_dst:
            return [
                IBnSid(xs, 0, xd, 1, offset, self.ctx),
                IOtAddi(xs, xs, 1, self.ctx),
            ]
        else:
...
class ILd(GIStd):
    ...
    def convert_otbn(self, addr):
        if self.MNEM.get(self.dmem_src) == "ld":
            ...
            if self.inc_src and self.inc_dst:
                return [
                    IBnLid(xd, 0, xd, 1, offset, self.ctx),
                    IOtAddi(xd, xd, 1, self.ctx),
                ]
            else:
...
class IBranch(GIMidImm):
    ...
    def convert_otbn(self, addr):
        ...
        logging.warning(
            "Unknown opcode at address " + self.addr + ". Cannot convert instruction."
        )
        return None
...
class ILoop(GIMidImm):
    ...
    def convert_otbn(self, addr):
        if self.fun == self.FUN_INDIRECT:  # star/indirect case
            ...
            logging.info(
                "OTBN conversion: Mapping lc limb "
                + str(self.limb)
                + " on GPR x"
                + str(gpr)
            )
