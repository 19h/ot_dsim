...
# C extension ABI version expected by this Python wrapper.
_C_MACHINE_ABI_VERSION = 1
...
# Try to import the C-accelerated Machine core.
# If available, Machine will inherit from CMachine (pure C state management)
# and only add the Python-level debug/display/stat helpers on top.
_FORCE_PURE_PYTHON = _env_truthy("OT_DSIM_PURE_PYTHON")

if _FORCE_PURE_PYTHON:
    _CMachineBase = None
    _CCallStackUnderrun = None
    _USE_C_MACHINE = False
else:
    try:
        from ot_dsim import _machine as _machine_mod

        _abi_version = getattr(_machine_mod, "ABI_VERSION", None)
        if _abi_version != _C_MACHINE_ABI_VERSION:
            raise ImportError(
                "ot_dsim._machine ABI mismatch: "
                f"expected {_C_MACHINE_ABI_VERSION}, got {_abi_version}"
            )

        _CMachineBase = _machine_mod.CMachine
        _CCallStackUnderrun = _machine_mod.CallStackUnderrun
        _USE_C_MACHINE = True
    except Exception:
        _CMachineBase = None
        _CCallStackUnderrun = None
...
# Use the C module's exception when available so that except clauses catch
# both C-raised and Python-raised underruns with a single type.
if _CCallStackUnderrun is not None:
    CallStackUnderrun = _CCallStackUnderrun
else:
...
class _PyMachine(object):
    """Pure-Python Machine implementation (original code, used as fallback)."""
    ...
    def stat_record_instr(self, instr):
        ins_str = (
            (instr.get_asm_str()[1]).split(" ", 1)[0].strip()
        )  # There seems to be no nicer way?
    ...
    def __init__(
        self, dmem, imem, s_addr=0, stop_addr=None, ctx=None, breakpoints=None
    ):
    ...
    def reset(self, dmem, imem, s_addr=0, stop_addr=None, clear_regs=False):
        ...
        if clear_regs:
            self.clear_regs()
        self.r_valid_half_limbs = [
            [False] * self.LIMBS * 2 for i in range(self.NUM_REGS)
        ]
        self.dmem = []
        self.imem = []
        self.init_dmem = []
    ...
    def __mod_half_limb_in_reg_val(self, lidx, regval, halflimbval, upper):
        ...
        masked_reg2 = masked_reg ^ mask
        reg = masked_reg2 | (
            halflimbval << ((lidx * 2 + bool(upper)) * self.half_limb_width)
        )
        return reg
    ...
    def set_reg_limb(self, ridx, lidx, value):
        ...
        self.set_reg(
            ridx,
            self.__mod_limb_in_reg_val(lidx, self.get_reg(ridx), value),
            valid_limb=lidx,
        )
    ...
    def set_reg_half_limb(self, ridx, lidx, value, upper):
        ...
        self.set_reg(
            ridx,
            self.__mod_half_limb_in_reg_val(lidx, self.get_reg(ridx), value, upper),
        )
    ...
    def set_reg_half_word(self, ridx, hw_idx, value):
        """Set a single limb in a register"""
        self.set_reg(
            ridx, self.__mod_half_word_in_reg_val(hw_idx, self.get_reg(ridx), value)
        )
    ...
    def get_dmem(self, address):
        ...
        if not self.init_dmem[address]:
            print(
                "Warning: reading from uninitialized dmem memory address: "
                + hex(address)
            )
    ...
    def set_dmem_otbn(self, address, value):
        ...
        self.__check_dmem_addr(dmem_addr)
        self.__check_reg_val(value)
        self.dmem[dmem_addr] = self.__mod_limb_in_reg_val(
            limb, self.dmem[dmem_addr], value
        )
        self.init_dmem[address] = True
    ...
    def get_reg_table(self, header):
        ...
        for i in range(0, self.NUM_REGS):
            if (i % 4) == 0:
                res_str += "\n"
            res_str += (
                ("r" + str(i)).rjust(3) + ": " + self.get_xlen_hex_str(self.get_reg(i))
            )
            if i != self.NUM_REGS:
                res_str += "\n"
        return res_str
    ...
    def get_all_flags_table(self):
        ...
        res_str += (
            "|"
            + str(int(self.get_flag("C")))
            + "|"
            + str(int(self.get_flag("Z")))
            + "|"
            + str(int(self.get_flag("M")))
            + "|"
            + str(int(self.get_flag("L")))
            + "|"
        )
        res_str += (
            "   |"
            + str(int(self.get_flag("XC")))
            + "|"
            + str(int(self.get_flag("XZ")))
            + "|"
            + str(int(self.get_flag("XM")))
            + "|"
            + str(int(self.get_flag("XL")))
            + "|"
        )
        return res_str
    ...
    def get_breakpoints(self):
        ...
        for key in self.breakpoints:
            ret_str += (
                ...
                + str(key)
                + ", stop at pass: "
                + str(self.breakpoints[key][0])
                + ", passed: "
                + str(self.breakpoints[key][1] - 1)
                + "\n"
            )
        return ret_str
    ...
    def __set_force_break(
        self,
        consider_callstack=False,
        callstack=0,
        consider_loopstack=False,
        loopstack=0,
    ):
        self.force_break = (
            True,
    ...
    def __handle_break_command(self, passes):
        ...
        while 1:
            ...
            if inp == "h":
                ...
                self.__set_force_break(
                    consider_callstack=True, callstack=len(self.call_stack)
                )
                break
            elif inp == "ol":
                if len(self.loop_stack) <= 0:
                    ...
                    self.__set_force_break(
                        consider_loopstack=True, loopstack=len(self.loop_stack) - 1
                    )
                    break
            elif inp == "o":
                if len(self.call_stack) <= 0:
                    ...
                    self.__set_force_break(
                        consider_callstack=True, callstack=len(self.call_stack) - 1
                    )
                    break
            elif inp == "r":
                ...
                if len(dmem_cmd) == 1 and dmem_cmd[0] == "d":
                    ...
                    if not (dmem_cmd[1].isdigit() and dmem_cmd[2].isdigit()):
                        ...
                        print(
                            self.get_dmem_table(
                                int(dmem_cmd[2]),
                                int(dmem_cmd[2]) + int(dmem_cmd[1]) - 1,
                            )
                        )
                else:
                    print("Invalid print dmem command")
...
if _USE_C_MACHINE:
    ...
    class Machine(_CMachineBase):
        """C-accelerated Machine with Python-level debug/display helpers."""

        # Class-level constants (must be plain ints for code that accesses
        # them on the class without an instance, e.g. Machine.NUM_REGS).
        NUM_REGS = 32
        NUM_GPRS = 32
        I_TYPE_IMM_WIDTH = 12
        XLEN = 256
        ...
        # Re-export the exception so import paths stay the same
        # CallStackUnderrun is already defined at module level

        def __init__(
            self, dmem, imem, s_addr=0, stop_addr=None, ctx=None, breakpoints=None
        ):
            super().__init__(dmem, imem, s_addr, stop_addr, ctx, breakpoints)

        # ---- stat recording helpers (lightweight, stay in Python) ----
        ...
        def stat_record_loop(
            self, loop_addr, loop_len, new_loop_stack_depth, iterations
        ):
            if "loops" not in self.stats:
                self.stats["loops"] = []
            self.stats["loops"].append(
        ...
        def get_reg_table(self, header):
            ...
            for i in range(0, self.NUM_REGS):
                ...
                res_str += (
                    ("r" + str(i)).rjust(3)
                    + ": "
                    + self.get_xlen_hex_str(self.get_reg(i))
                )
                if i != self.NUM_REGS:
                    res_str += "\n"
            return res_str
        ...
        def dump_dmem(self, length, filename):
            ...
            for i in range(0, min(length, self.DMEM_DEPTH)):
                f.write(
                    str(i).zfill(4) + ": " + self.get_xlen_hex_str(self.dmem[i]) + "\n"
                )
            f.close()
        ...
        def __handle_break_command(self, passes):
            """Interactive breakpoint handler - stays in Python for I/O."""
            if passes:
                print(
                    "Breakpoint hit at address "
                    + str(self.get_pc())
            ...
            while 1:
                ...
                if inp == "h":
                    ...
                    self.force_break = (True, False, 0, False, 0)
                    break
                elif inp == "n":
                    self.force_break = (True, True, len(self.call_stack), False, 0)
                    break
                elif inp == "ol":
                    if len(self.loop_stack) <= 0:
                        print('Nothing on loop stack, can\'t "step out".')
                    ...
                    if len(dmem_cmd) == 1:
                        ...
                        if dmem_cmd[1].isdigit():
                            print(self.get_dmem_table(0, int(dmem_cmd[1]) - 1))
                    elif len(dmem_cmd) == 3:
                        if dmem_cmd[1].isdigit() and dmem_cmd[2].isdigit():
                            print(
                                self.get_dmem_table(
                                    int(dmem_cmd[2]),
                                    int(dmem_cmd[2]) + int(dmem_cmd[1]) - 1,
                    ...
                    if len(p_cmd) == 1:
                        ...
                        self.toggle_breakpoint(p_cmd[1], msg=True)
                    elif len(p_cmd) == 3 and p_cmd[2].isdigit():
                        self.toggle_breakpoint(p_cmd[1], int(p_cmd[2]), msg=True)
                elif inp.split()[0] == "dump":
                    cmd = inp.split()
                    if len(cmd) == 2 and cmd[1].isdigit():
                        self.dump_dmem(int(cmd[1]), self.DEFAULT_DUMP_FILENAME)
                    elif len(cmd) == 3 and cmd[1].isdigit():
                        self.dump_dmem(int(cmd[1]), cmd[2])
                else:
                    print("Invalid command.")
    ...
    # Fallback: use the pure-Python implementation
    Machine = _PyMachine
